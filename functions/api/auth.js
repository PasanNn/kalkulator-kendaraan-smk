...
export async function onRequest(context) {
  ...
  try {
    ...
    return json({ok:false,message:'D1 belum terhubung. Periksa binding DB di Cloudflare Pages.'},500);
  }
  ...
  if (action === 'login' && request.method === 'POST') {
    ...
    const u = await env.DB.prepare(`SELECT id,username,password_hash,role FROM app_users WHERE username=? LIMIT 1`).bind(username).first();
    ...
    await env.DB.prepare(`INSERT INTO app_sessions(user_id,token_hash,expires_at) VALUES(?,?,datetime('now','+7 days'))`)
      .bind(Number(u.id), tokenHash).run();

    return json({ok:true,user:publicUser(u)},200,{'Set-Cookie':cookie(COOKIE,token,SESSION_DAYS*86400)});
  }
  ...
  if (!user) return json({ok:false,message:'Sesi login tidak ditemukan atau sudah kedaluwarsa.'},401);
  if (user.role !== 'admin') return json({ok:false,message:'Hanya admin yang boleh mengelola akun.'},403);
  ...
  if (action === 'list_users' && request.method === 'GET') {
    ...
    return json({ok:true,users:rows.results.map(r=>({id:Number(r.id),username:r.username,role:r.role}))});
  }
  ...
  if (action === 'add_user' && request.method === 'POST') {
    ...
    if (!/^[A-Za-z0-9._-]{3,80}$/.test(username)) return json({ok:false,message:'Username 3-80 karakter, gunakan huruf/angka/titik/underscore/minus.'},422);
    if (password.length < 6) return json({ok:false,message:'Password minimal 6 karakter.'},422);
    try {
      const hash = await hashPassword(password);
      await env.DB.prepare(`INSERT INTO app_users(username,password_hash,role) VALUES(?,?,?)`).bind(username,hash,role).run();
      return json({ok:true});
    } catch { return json({ok:false,message:'Username sudah digunakan.'},409); }
  }
  ...
  if (action === 'update_user' && request.method === 'POST') {
    ...
    try {
      if (password) {
        if (password.length < 6) return json({ok:false,message:'Password minimal 6 karakter.'},422);
        const hash = await hashPassword(password);
        await env.DB.prepare(`UPDATE app_users SET username=?,password_hash=?,role=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(username,hash,role,id).run();
      } else {
        await env.DB.prepare(`UPDATE app_users SET username=?,role=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(username,role,id).run();
      }
      return json({ok:true});
    } catch { return json({ok:false,message:'Username sudah digunakan.'},409); }
  }
